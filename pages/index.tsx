...
const filteredAssets = assets.filter(
  (asset) =>
    asset.chain_name !== "cryptoorgchain" &&
    chainNames.includes(asset.chain_name)
);
...
export default function Home() {
  ...
  const getAllValues = () => {
    cw20Client?.queryAllValues().then((val) => {
      const splitVal = val[val.length - 1].split(":")[1];
  ...
  return (
    <Container maxW="5xl" py={10}>
      ...
      <Box textAlign="center">
        ...
        <Heading fontSize={{ base: "lg", sm: "md", md: "md" }}>
      ...
      <Grid
        templateColumns={{ md: "repeat(auto-fill, 22%)" }}
      ...
      >
        {filteredAssetsBySearch.map((asset, index) => (
          <Dependency
            key={asset.chain_name + index.toString()}
            {...asset.assets[0]}
            {...{ price: prices[index] }}
          ></Dependency>
        ))}
